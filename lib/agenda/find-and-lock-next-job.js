...
module.exports = function (jobName, definition, cb) {
  ...
  if (s.topology.connections().length === 0) {
    ...
    (async function () {
      try {
        let res = await this._collection.updateMany(JOB_PROCESS_WHERE_QUERY, JOB_PROCESS_SET_QUERY);
        console.log('res.matchedCount', res.matchedCount);
        console.log('res.modifiedCount', res.modifiedCount);

        let jobs = await this._collection.find({lockedUuid}).sort(this._sort).toArray();
